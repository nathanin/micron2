...
full_sample_id = f'210115_Breast_Cassette15_reg1'
...
tools = [hover_tool, PanTool(), ResetTool(), WheelZoomTool(), BoxSelectTool()]
...
boxplot_module = BokehBoxPlot(shared_variables['adata_data'], 
                              varnames=[f'{v}_nuclei_mean' for v in shared_variables['all_channels'] if v !='DAPI'],
...
def update_scatter():
  ...
  figure_sources['scatter_fg'].data = dict(
    ...
    training=shared_variables['adata_data']['training_labels'], # have to change this to allow switching the shown annotations
    predict_prob=shared_variables['adata_data']['predicted_proba'], # have to change this to allow switching the shown annotations
