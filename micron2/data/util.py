...
def _merge_image_datasets(h5fout, h5fs, dataset, channel, size, dtype=np.uint8, perm=None):
  ...
  for h5f in h5fs:
    with h5py.File(h5f,'r') as fin:
      try:
        total_size += fin[f'{dataset}/{channel}'].shape[0]
  ...
  dset = h5fout.create_dataset(f'{dataset}/{channel}', 
                                  shape=(total_size, size, size), 
                                  compression='gzip', 
                                  dtype=dtype,
                                  chunks=(1,size,size))
  ...
  # data = []
  i = 0
  with tqdm.tqdm(h5fs) as pbar:
    for h5f in pbar:
      pbar.set_description(f'{dataset}/{channel}: {os.path.basename(h5f)}')
      with h5py.File(h5f, 'r') as fin:
        data = fin[f'{dataset}/{channel}'][:]

      for d in data:
        if perm is not None:
          loc = np.argwhere(perm==i).flatten()[0]
        else:
          loc = i
        dset[loc:loc+1,:,:] = d
        i+=1
...
def hdf5_concat(h5fs, h5out, channels='all', mode='a', sample_layout=None, dry_run=False):
  ...
  with h5py.File(h5out, mode) as h5fout:
    ...
    print('Using cell perm:', cell_perm.shape)
    for ch in channels:
      _merge_image_datasets(h5fout, h5fs, 'cells', ch, cell_size, dtype=np.uint8, perm=cell_perm)
      h5fout.flush()

    image_size = _get_size_attr(h5fs[0], 'images')
    image_perm = h5fout['meta/image_samples_perm'][:]
    print('Using image perm:', image_perm.shape)
    for ch in channels:
      _merge_image_datasets(h5fout, h5fs, 'images', ch, image_size, dtype=np.uint8, perm=image_perm)
      h5fout.flush()
    ...
    _merge_image_datasets(h5fout, h5fs, 'meta', 'nuclear_masks', cell_size,  dtype=np.bool, perm=cell_perm)
    _merge_image_datasets(h5fout, h5fs, 'meta', 'membrane_masks', cell_size, dtype=np.bool, perm=cell_perm)
